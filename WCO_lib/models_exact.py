    ...
    def set_up_model(self):
        ...
        for edge in self.problem_params.existing_edges:
            if not edge[0] in touched_vertices:
                ...
                touched_vertices_no_origins.append(edge[0])
            if not edge[1] in touched_vertices:
                touched_vertices.append(edge[1])
                touched_vertices_no_origins.append(edge[1])

        if 0 in touched_vertices_no_origins:
            touched_vertices_no_origins.remove(0)
        ...
        for t in range(T):
            for k in range(K):
                self.model.addConstr(gb.quicksum(self.x[i, j, k, 0, t] for (i, j) in self.problem_params.existing_edges
                                     ...
                                     name=f"constraint (16) for period {t} and vehicle {k}")
        ...
        for t in range(T):
            for k in range(K):
                self.model.addConstr(gb.quicksum(self.x[i, j, k, 0, t] for (i, j) in self.problem_params.existing_edges
                                     ...
                                     name=f"constraint (17) for period {t} and vehicle {k}")
